...
The Date column is explicitly converted to a datetime type to enable proper time-based operations.
The data is then sorted by City and Date. This is critical because later steps (lags and rolling statistics) assume chronological order within each city.

**2. Identification of numerical columns**

All columns with numeric data types are programmatically identified.
This allows preprocessing logic to be applied consistently across all numerical features without hardcoding column names.

**3. Missing value handling (group-aware interpolation)**

For every numerical column:

Missing values are interpolated within each city group, preserving local temporal trends.

Any remaining missing values after interpolation are filled with the median value of that column for the same city.

This ensures:

No information leakage across cities

Trends are preserved where possible

A deterministic fallback when interpolation fails at boundaries

**4. Explicit NaN replacement for selected pollutants**

Specific pollutant columns (PM10, NOx, NH3, O3, Benzene, Toluene, Xylene) undergo an additional replacement step where remaining NaN values are filled with predefined constant values.

These constants appear to be precomputed global or domain-specific statistics and act as a final safety net against missing data.

**5. Column removal**

The categorical column AQI_Bucket is dropped entirely.
This indicates the model is designed for numerical regression on AQI, not classification.

**6. Time-series feature engineering**

Several temporal features are created per city:

AQI_lag_1: AQI value from the previous day

AQI_lag_7: AQI value from seven days prior

AQI_roll_7: 7-day rolling mean of AQI

These features explicitly encode short-term memory and weekly trends, which are essential for sequence models like LSTMs.

**7. Row-wise NaN cleanup after feature engineering**

Lag and rolling operations naturally introduce missing values at the start of each city’s time series.
All rows containing NaN values after feature creation are dropped to ensure the model only sees fully-defined samples.

**8. Feature–target separation**

A predefined list of feature columns (FEATURES) and a target column (TARGET, AQI) is used to split the dataset into:

Input features (X)

Prediction target (y)

This separation is done explicitly before scaling.

**9. Feature scaling**

Two independent MinMaxScaler instances are used:

One scaler for input features (X)

One scaler for the target variable (y)

Both are fit on the full dataset and transform values into the [0, 1] range.
This is essential for LSTM stability and gradient behavior.

**10. Time-based train–test split**

The dataset is split sequentially, not randomly:

First 80% of samples → training set

Remaining 20% → test set

This preserves temporal order and avoids future data leaking into the training set, which is mandatory for time-series modeling.

**11. Persistence of preprocessing artifacts**

The fitted feature scaler and target scaler are saved to disk using joblib.
This guarantees that the exact same preprocessing steps can be applied during inference or deployment.
